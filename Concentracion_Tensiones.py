...
def factores_teoricos(case, D, d, r):
    ...
    if case == 1:
        ...
        for i in diameter_ratio:

            if nearest_value is None or abs(i - diam_ratio) < abs(nearest_value - diam_ratio):
                nearest_value = diam_ratio
                nearest_position = i

            elif abs(i - diam_ratio) == abs(nearest_value - diam_ratio) and i > nearest_value:
                nearest_value = diam_ratio
                nearest_position = i
        ...
        kt_4 = [2.60, 2.10, 1.82, 1.75, 1.60, 1.55, 1.45, 1.40]
